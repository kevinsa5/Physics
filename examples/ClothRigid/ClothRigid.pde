...
void setup(){
  ...
  for(int i = 0; i < xnum; i++){
    for(int j = 0; j < ynum; j++){
      if(i!=0) sys.addRigidBody(new RigidBody(nodes[j][i],nodes[j][i-1]));
      if(j!=0) sys.addRigidBody(new RigidBody(nodes[j][i],nodes[j-1][i]));
      if(i!=xnum-1) sys.addRigidBody(new RigidBody(nodes[j][i],nodes[j][i+1]));
      if(j!=ynum-1) sys.addRigidBody(new RigidBody(nodes[j][i],nodes[j+1][i]));
